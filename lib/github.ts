...
import { buildBlogAuditRecord } from './blog-audit';
...
export async function createDraftPR(input: CreateDraftPRInput): Promise<CreateDraftPRResult> {
  ...
  const auditPath = `content/blog-audits/${input.slug}.audit.json`;
  const auditRecord = buildBlogAuditRecord({
    slug: input.slug,
    lane: input.lane,
    categoryId: typeof input.metadata?.category_id === 'string' ? input.metadata.category_id : input.bundle.category_id,
    bundle: input.bundle,
    gateReport: input.gateReport,
    markdownContent: input.markdownContent,
    seoAeoScore: input.seoAeoScore,
    heroFallbackUsed: input.heroFallbackUsed,
  });
  await upsertFile(
    branchName,
    auditPath,
    JSON.stringify(auditRecord, null, 2),
    `feat(blog): add audit for ${input.slug}`,
  );

  // 5. Upload metadata (gate report + bundle summary)
  if (input.metadata) {
  ...
  const body = [
    ...
    gatesSummary,
    '',
    '### Publish Guardrail Audit',
    `- Audit file: \`${auditPath}\``,
    '- Site CI runs `npm run guard:blog -- --publish` against this post before merge.',
    '- Final human publish approval still happens through GitHub PR review and merge.',
    '',
    imageOptionsSection,
    '---',
  ...
  // 7. Add labels (best-effort, don't fail if labels don't exist)
